...
var _ = Describe("Poller", func() {
	...
	BeforeEach(func() {
		...
		validator = drainvalidation.NewValidator(&dummyIPChecker{}, 120*time.Second)
	})

	It("polls for bindings on an interval", func() {
		p := NewPoller(apiClient, 10*time.Millisecond, store, metrics, logger, appLogStream, validator, false)
		go p.Poll()
	...
	It("tracks the number of bindings returned from CAPI", func() {
		...
		NewPoller(apiClient, time.Hour, store, metrics, logger, appLogStream, validator, true)
	...
	It("filters invalid bindings", func() {
		...
		NewPoller(apiClient, time.Hour, store, metrics, logger, appLogStream, validator, false)
	...
	Describe("checkBindings", func() {
		var newTestPoller func(checker drainvalidation.IPChecker) *Poller

		BeforeEach(func() {
			newTestPoller = func(checker drainvalidation.IPChecker) *Poller {
				v := drainvalidation.NewValidator(checker, 120*time.Second)
				// Build a Poller without calling poll() by constructing directly
				opt := goMetrics.WithMetricLabels(map[string]string{"unit": "total"})
				return &Poller{
					apiClient:       apiClient,
					pollingInterval: time.Hour,
					store:           store,
					logger:          logger,
					bindingRefreshErrorCounter: metrics.NewCounter(
						"binding_refresh_error",
						"Total number of failed requests to the binding provider.",
					),
					lastBindingCount: metrics.NewGauge(
						"last_binding_refresh_count",
						"Current number of bindings received from binding provider during last refresh.",
					),
					invalidDrains: metrics.NewGauge(
						"invalid_drains",
						"Count of invalid drains encountered in last binding fetch. Includes blacklisted drains.",
						opt,
					),
					blacklistedDrains: metrics.NewGauge(
						"blacklisted_drains",
		...
		Context("when source_type is present", func() {
			It("returns no binding which contains an invalid URL", func() {
				bindings := []Binding{
					{
						Url: "syslog:/drain-0",
						Credentials: []Credentials{
				...
				p := newTestPoller(&dummyIPChecker{})
				...
				Expect(metrics.GetMetricValue("invalid_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 0))
				Expect(metrics.GetMetricValue("blacklisted_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 0))
			})
			...
			It("returns no binding with unresolvable URL", func() {
				...
				p := newTestPoller(blacklistRanges)
				filteredBindings := p.checkBindings(bindings)
				...
				Expect(metrics.GetMetricValue("invalid_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 1))
				Expect(metrics.GetMetricValue("blacklisted_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 0))
			})
			...
			It("returns no binding which has a blacklisted IP", func() {
				...
				Expect(metrics.GetMetricValue("invalid_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 1))
				Expect(metrics.GetMetricValue("blacklisted_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 1))
			})
			...
			It("returns no binding when there is a prior IP checking failure for URL", func() {
				...
				Expect(metrics.GetMetricValue("invalid_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 2))
				Expect(metrics.GetMetricValue("blacklisted_drains", map[string]string{"unit": "total"})).To(BeNumerically("==", 0))
			})
